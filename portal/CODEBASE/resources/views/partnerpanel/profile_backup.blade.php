                ...
                <div class="card-body p-0">
                    ...
                    <div class="d-flex rating-section text-center">
                        <div class="col-lg-4">
                            <p>
                                <span>0 </span>/5 <br>
                                <label class="mt-0">0 Ratings</label>
                            </p>
                        </div>
                        <div class="col-lg-4">
                            <p>
					...
					<div class="col-lg-2 col-md-3 pr-0">
						...
						@if($vendor->status == 1)
						  ...
						  @elseif($vendor->status == 3) 
						   <span class="badge badge-danger mb-0">Profile Not Activated, On Hold </span></label>
						    <label class=""> <b>Reason :</b> 
							<p class="">{{$vendor->hold_message}} </p></label> 
							@elseif($vendor->status == 0)
							<span class="badge badge-danger mb-0">Profile Not Activated : Status Pending</span> @endif </div>
                            ...
                            <div class="row ">
                                ...
                                <div class="col-lg-10 col-md-10">
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="address" disabled="disabled" value="{{$vendor->address1}},{{$vendor->address2}},{{$vendor->landmark}}-{{$vendor->pincode}}." />
                                    </div>
                                </div>
                                <div class="col-lg-2 col-md-3 pr-0">
                                ...
                                <div class="col-lg-10 col-md-10">
                                    <fieldset class="form-label-group">
                                        <textarea class="form-control" disabled='true' id="aboutus" rows="4"> {{$vendor->about_us}}</textarea>
                                    </fieldset>
                                </div>
                                <div class="col-lg-2 col-md-3 pr-0">
                                    <label>Services</label>
                                </div>
                                <div class="col-lg-10 col-md-10"> @php $vdata = json_decode($vendor->services,TRUE); @endphp @foreach($vdata as $data) <div class="badge bg-light-warning rounded mr-1 mt-0">{{$data}}</div> @endforeach </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row justify-content-center pt-0">
        <div class="col-12">
            <div class="card">
                ...
                <hr />
                <div class="row "> @php $j=1; $projects = App\Models\Vendor_Project_info::where('vendor_ref_id',$vendor->id)->whereNull('deleted_at')->get(); @endphp @if(count($projects) > 0) @foreach($projects as $projects) <div class="col-sm-12">
                        <div class="card border">
                            <div class="card-header py-1">
                                <!-- <div class="badge badge-secondary py-10" >Project Name : {{$projects->projectname}}</div> --> @php $categoryval = App\Models\Category::where('id',$projects->category)->first(); $subcategoryval = App\Models\ChildCategory::where('id',$projects->subcategory)->where('status',1)->first(); @endphp <div class="badge badge-secondary customcls py-10">CATEGORY NAME : {{$categoryval->name}}</div> @php if($projects->subcategory !=''){ @endphp <div class="badge badge-secondary customcls py-10">SUB-CATEGORY NAME : @php echo $subcategoryval->name; @endphp </div> @php } @endphp <div>
                                ...
                                <div class="">
                                    <div class=" slider-for">
                                        <!-- <div id="slider-container{{$j}}" class="slider"> -->
                                         @if($projects->images != '') @php $da = json_decode($projects->images,TRUE); $i=0; @endphp @foreach($da as $img) <div class="slide">
                                            <div class="slider-inner">
                                                ...
                                                <div class="nav-item dropdown dropdown-user actions-btn">
                                                    <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                        <div class="user-nav d-md-flex d-none"><i data-feather="more-vertical"></i>
                                                        </div>
                                                    </a>
                    ...
                    </div> @php $j++; @endphp @endforeach @endif @php $deleted = json_decode($vendor->deleted_services,TRUE); @endphp @php $categories = App\Models\Category::where('status',1)->whereNull('deleted_at')->orderby('name','asc')->get(); @endphp @foreach($categories as $cate) @if(in_array($cate->name,$vdata)) @if(!in_array($cate->id,$deleted)) <div class="col-sm-12">
                        <div class="card border">
                            <div class="card-header py-1">
                                <div class="badge badge-secondary customcls py-10">CATEGORY NAME : {{$cate->name}} </div>
                                <div> @if(in_array($cate->name,$vdata)) <i data-feather="upload" data-toggle="modal" title="Upload Images" data-target="#uploadimage" id="{{$cate->id}}" onclick="uploadimg(this)" class="mr-1 icon-border"></i> @else <i data-feather="upload" data-toggle="modal" title="Upload Images" data-target="#viewdetail" onclick="changestatus(2)" class="mr-1 icon-border"></i> @endif <i data-feather="trash-2" class="icon-border trash-bg" title="Delete Category" id="{{$cate->id}}" onclick="deletecategory(this)"></i>
                                </div>
                            </div>
                            <div class="card-body text-center">
                                <div class="">
                    ...
                    </div> @endif @endif @endforeach </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="profile-picture mx-auto mb-2">
                                <div class="profile-img"> @if($vendor->photo !='') <img src="{{$vendor->photo}}" class="round img-fluid" id='output' alt="Card image"> @else <img src="{{url('partner-assets/app-assets/images/avatars/profile.png')}}" class="round img-fluid" id='output' alt="Card image"> @endif </div>
                            </div>
                        </div>
                        <div class="col-12">
                        ...
                        <div class="col-12">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" required id="noofprojects" class="form-control" name="noofprojects" placeholder="No of Projects" value='{{$vendor->no_of_projects}}' />
            ...
            <form class="form form-horizontal " id="addi-save" method="post" action="{{route('ppanel.updateadditionaldetail')}}" enctype="multipart/form-data"> {!! csrf_field() !!} <div class="modal-body ">
                    <div class="row">
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <fieldset class="form-label-group">
                                <textarea class="form-control" id="aboutus" maxlength="255" name="aboutus" rows="5"> {{$vendor->about_us}}</textarea>
                            </fieldset>
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <select class="floating-select select2 form-control form-control dt-input dt-full-name" data-column="1" placeholder="Primary Category" required data-column-index="0" multiple id="category" name="category[]">
                                <option value="">Select Category</option> @foreach($categories as $category) @if(in_array($category->name,$vdata)) <option value="{{$category->name}}" selected>{{$category->name}}</option> @else <option value="{{$category->name}}">{{$category->name}}</option> @endif @endforeach
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <form class="form form-horizontal " id="project-save" method="post" action="{{route('ppanel.addcategory')}}" enctype="multipart/form-data"> {!! csrf_field() !!} <div class="modal-body ">
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <select class=" form-control" onchange='getsubcategories()' placeholder="Primary Category" id="categoryserve" name="categoryserve">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-success waves-effect waves-float waves-light" name="saveproject" id="saveproject" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Processing..."> Save </button>
                </div>
        </div>
        </form>
    </div>
    ...
    <div class="modal-dialog modal-md modal-dialog-centered" role="document">
        <div class="modal-content file-manager-application">
            ...
            <form class="form form-horizontal " id="project-saveimg" method="post" action="{{route('ppanel.addprojectimage')}}" enctype="multipart/form-data"> {!! csrf_field() !!} <input type=hidden id='category' name='category'>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-Success waves-effect waves-float waves-light" name="saveprojectimg" id="saveprojectimg" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Processing..."> Save </button>
                </div>
        </div>
        </form>
    </div>
    ...
    <div class="modal-dialog modal-md modal-dialog-centered" role="document">
        <div class="modal-content file-manager-application">
            ...
            <form class="form form-horizontal " id="project-editimg" method="post" action="{{route('ppanel.updateprojectimage')}}" enctype="multipart/form-data"> {!! csrf_field() !!} <input type=hidden id='project_id' name='project_id'>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-Success waves-effect waves-float waves-light" name="editimg" id="editimg" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Processing..."> Update </button>
                </div>
        </div>
        </form>
    </div>
    ...
    <div class="modal-dialog modal-md modal-dialog-centered" role="document">
        <div class="modal-content file-manager-application">
            ...
            <form class="form form-horizontal " id="project-uploadimg" method="post" action="{{route('ppanel.uploadprojectimage')}}" enctype="multipart/form-data"> {!! csrf_field() !!} <input type=hidden id='categoryid' name='categoryid'>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-Success waves-effect waves-float waves-light" name="uploadprojectimg" id="uploadprojectimg" data-loading-text="<i class='fa fa-spinner fa-spin'></i> Processing..."> Upload </button>
                </div>
        </div>
        </form>
    </div>
...
<div class="modal fade text-left" id="editproject" tabindexs="-4" aria-labelledby="myModalLabel6" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content file-manager-application">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-Success waves-effect waves-float waves-light" (click)="modal.close('Accept click')" rippleEffect> Update </button>
            </div>
        </div>
    </div>
</div>

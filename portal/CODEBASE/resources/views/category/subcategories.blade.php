                     ...
                     <div class="col-12">
                        <div class="card table-filters-design">
                                 ...
                                 <div class="col-3">
                                    ...
                                    <div class="col-3">
                                       <div class="form-row mt-2">
                                          ...
                                          <a href="{{route('childcategory')}}" class="btn btn-outline-secondary"><i class="fal fa-redo"></i></a>
                                       </div>
                                    </div>
                                 </div>
                              </form>
                           ...
                           <div class="card basic-datatable table-fixedheader datatable-list">
                              <table class="dt-subcategories table table-bordered table-responsive">
                                 <thead>
                                    <tr>
                                          <th>Sub Category</th>
...
<script type="text/javascript">
   var Subcategories = function () {
      return { //main function to initiate the module
         ...
         var dt_ajax_table = $('.dt-subcategories');
            ...
            if (dt_ajax_table.length) {
                var dt_ajax = dt_ajax_table.dataTable({
                    ...
                    buttons: [{
                     ...
                     text: feather.icons['plus'].toSvg({ class: 'font-small-4 mr-50' }) + 'Add New Sub-Category </a>',
                     attr:{
                        "href": "{{route('childcategory.addEditSubCategory')}}",
                        "data-toggle": "modal",
            ...
            $('.dt-subcategories').on('click', '.delete-record', function() {
                    var $this = $(this);
            ...
            $('body').on('submit', '#subcategory-save', function(e) {
                e.preventDefault();
                $.ajax({
                    type: "post",
                    contentType:false,
                    processData:false,
                    url: $('form.subcategory-save').attr('action'),
                    data: new FormData(this),
                    datatype: JSON,
                    success: function(response) {
                        if(response.status == 'validations') {
                            $.each(response.errors, function (key, error) {
                                $('form.subcategory-save').find("#" + key).removeClass('error').addClass('error');
                                $('form.subcategory-save').find("#" + key + "-error").text(error[0]);
                            });
                        }

                        if(response.status == 'success') {
                            window.location = "{{route('childcategory')}}"
                        }
            ...
            $('body').on('click', '#dt-feature-filter-form-search', function(e) {
                e.preventDefault();
                var url = "{{route('childcategory')}}?search=1";
